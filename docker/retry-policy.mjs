// Keep Docker configuration independent of the extracted Desktop bundle.
...
export function integerSetting(
  ...
  if (!Number.isSafeInteger(value) || value < minimum || value > maximum) {
    throw new Error(
      `${PREFIX}${name} must be an integer from ${minimum} to ${maximum}`,
    );
  }
  return value;
}
...
export function retryPolicy(env = process.env, warn = () => {}) {
  ...
  if (legacy.length && !modern) {
    ...
    const initial = integerSetting(
      env,
      "RECONNECT_BACKOFF_INITIAL_DELAY_MS",
      300_000,
      1,
    );
    const increment = integerSetting(
      env,
      "RECONNECT_BACKOFF_INCREMENT_MS",
      300_000,
    );
    if (initial + Math.max(0, backoff - 1) * increment > MAX_TIMER) {
      throw new Error(
        "configured reconnect backoff exceeds the supported timer range",
      );
    }
    return {
  ...
  const raw =
    env[PREFIX + "RETRY_DELAYS_MS"]?.trim() || "10000,10000,20000,30000";
